...
export default function Layout({ page, onNav, trades, user, onSignOut, onExport, onImport, darkMode, onToggleDark, accounts, activeAccountId, onSwitchAccount, onCreateAccount, onEditAccount, onDeleteAccount, startingBalance, children }) {
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'transparent', position:'relative' }}>
      ...
      {/* SIDEBAR */}
      <aside className="sidebar glass-panel" style={{ width:'var(--nav-w)', position:'fixed', top:'16px', left:'16px', height:'calc(100vh - 32px)', background:'var(--surface)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', zIndex:50, borderRadius:'24px' }}>
        <div style={{ padding:'20px 18px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:'10px' }}>
          ...
          <div><div className='gradient-text' style={{ fontSize:'18px', fontWeight:'800', letterSpacing:'-.04em' }}>TradeTracker</div><div style={{fontSize:'10px',color:'var(--muted)',letterSpacing:'.18em',textTransform:'uppercase'}}>Elite Trading OS</div></div>
        </div>
        ...
        <div style={{ padding:'8px', borderTop:'1px solid var(--border)', display:'flex', flexDirection:'column', gap:'6px' }}>
          ...
          <div style={{ display:'flex', gap:'4px' }}>
            <button className="btn btn-outline btn-sm hover-glow" style={{ flex:1, justifyContent:'center', fontSize:'10px' }} onClick={onExport}>↑ Export</button>
            <label className="btn btn-outline btn-sm hover-glow" style={{ flex:1, justifyContent:'center', cursor:'pointer', fontSize:'10px' }}>
      ...
      {/* MAIN */}
      <div className="main" style={{ marginLeft:'calc(var(--nav-w) + 24px)', flex:1, minHeight:'100vh', display:'flex', flexDirection:'column', padding:'16px' }}>
        <header className='glass-panel' style={{ background:'var(--surface)', border:'1px solid var(--border)', padding:'0 28px', height:'68px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:40, borderRadius:'24px', marginBottom:'18px' }}>
          <div><div style={{ fontSize:'11px', fontWeight:'700', color:'var(--muted)', letterSpacing:'.18em', textTransform:'uppercase' }}>Workspace</div><div className='gradient-text' style={{ fontSize:'24px', fontWeight:'800', lineHeight:1.1 }}>{NAV.find(n => n.id === page)?.label}</div></div>
          <div style={{ display:'flex', gap:'5px', alignItems:'center' }}>
            <button className="btn btn-icon btn-ghost" onClick={onToggleDark} style={{ fontSize:'13px' }}>{darkMode ? '○' : '●'}</button>
            <button className="btn btn-outline btn-sm hover-glow" onClick={onExport}>Export</button>
            <label className="btn btn-outline btn-sm hover-glow" style={{ cursor:'pointer' }}>Import<input type="file" accept=".json" style={{ display:'none' }} onChange={onImport} /></label>
          </div>
        </header>
