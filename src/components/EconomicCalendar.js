...
export default function EconomicCalendar() {
  ...
  function refresh() {
    sessionStorage.removeItem('tt_econ_fresh1')
  ...
  return (
    <div className="page active">
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'16px', flexWrap:'wrap', gap:'10px' }}>
        <div>
          ...
          <div style={{ fontSize:'11px', color:'var(--muted)', fontWeight:'600' }}>
            🔴 High impact · USD · GBP · EUR · This week
          </div>
        </div>
        <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
          {[['🇺🇸','USD',usd],['🇬🇧','GBP',gbp],['🇪🇺','EUR',eur]].map(([flag,cur,n]) => (
            <div key={cur} style={{ display:'flex', alignItems:'center', gap:'4px', padding:'4px 10px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'6px', fontSize:'11px', fontWeight:'700' }}>
              <span>{flag}</span>
              <span style={{ color: CCY[cur]?.text }}>{cur}</span>
          ...
          <button onClick={refresh} style={{ background:'none', border:'1px solid var(--border)', borderRadius:'6px', padding:'4px 10px', cursor:'pointer', fontSize:'12px', fontWeight:'600', color:'var(--muted)', fontFamily:'inherit' }}>
            ↻ Refresh
          </button>
        </div>
      </div>
      ...
      {/* Error */}
      {!loading && error && (
        <div style={{ padding:'14px 16px', background:'var(--red-bg)', border:'1px solid var(--red-dim)', borderRadius:'var(--r)', color:'var(--red)', fontSize:'13px', fontWeight:'600', marginBottom:'14px' }}>
          ⚠️ {error} &mdash; <button onClick={refresh} style={{ color:'var(--blue)', background:'none', border:'none', cursor:'pointer', fontWeight:'700', fontSize:'13px', fontFamily:'inherit' }}>Try again</button>
        </div>
      )}
      ...
      {!loading && (
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--r)', overflow:'hidden', boxShadow:'var(--shadow)' }}>
          ...
          {/* Column headers */}
          <div style={{ display:'grid', gridTemplateColumns:'110px 72px 80px 28px 1fr 88px 88px 88px', background:'var(--surface2)', borderBottom:'2px solid var(--border2)' }}>
            {['Date','Time (EST)','Currency','','Event','Actual','Forecast','Previous'].map((h,i) => (
              <div key={i} style={{ padding:'8px 10px', fontSize:'10px', fontWeight:'700', color:'var(--muted)', letterSpacing:'.06em', textTransform:'uppercase', textAlign: i >= 5 ? 'center' : 'left' }}>
                {h}
              </div>
            ))}
          </div>
          ...
          {weekDays.map((day, di) => {
            ...
            return (
              <div key={day.dateStr} style={{ borderLeft: isToday ? '3px solid var(--amber)' : '3px solid transparent', borderBottom: di < 6 ? '1px solid var(--border)' : 'none' }}>
                {dayEvs.length === 0 ? (
                  <div style={{ display:'grid', gridTemplateColumns:'110px 72px 80px 28px 1fr 88px 88px 88px', alignItems:'center', minHeight:'38px', background: bg, opacity: day.isWeekend ? .55 : 1 }}>
                  ...
                  return (
                    <div key={ei} style={{ display:'grid', gridTemplateColumns:'110px 72px 80px 28px 1fr 88px 88px 88px', alignItems:'center', minHeight:'42px', background: isToday ? 'rgba(251,191,36,.04)' : bg, borderTop: ei > 0 ? '1px solid var(--border)' : 'none', transition:'background .1s' }}
      ...
      <div style={{ textAlign:'center', marginTop:'10px', fontSize:'10px', color:'var(--muted2)' }}>
        Data from <a href="https://www.forexfactory.com" target="_blank" rel="noopener noreferrer" style={{ color:'var(--blue)', textDecoration:'none', fontWeight:'600' }}>ForexFactory.com</a> · Updates each Monday · High impact USD/GBP/EUR only
      </div>
    </div>
  )
}
