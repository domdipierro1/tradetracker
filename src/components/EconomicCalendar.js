    ...
    <div className="page active">
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'14px', flexWrap:'wrap', gap:'10px' }}>
        ...
        <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
          ...
          <button className="btn btn-outline btn-sm" onClick={()=>{ sessionStorage.removeItem('tt26_econ_v5'); window.location.reload() }}>↻</button>
        </div>
      </div>
      ...
      {!loading && error && (
        <div style={{ padding:'12px 14px', background:'var(--red-bg)', border:'1px solid var(--red-dim)', borderRadius:'var(--r)', color:'var(--red)', fontSize:'13px', fontWeight:'600' }}>
          ⚠️ {error} — <button onClick={()=>{ sessionStorage.removeItem('tt26_econ_v5'); window.location.reload() }} style={{ color:'var(--blue)', background:'none', border:'none', cursor:'pointer', fontWeight:'700', fontFamily:'inherit', fontSize:'13px' }}>Retry</button>
        </div>
      )}
